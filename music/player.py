...
class Player:
    def __init__(self):
        ...
        # self.info = {'playing_volume':50,}
        self.playing_volume = 80
        self.songs = [] #SongInfo list
        self.playing_idx = 0
        # self.playing_id = -1
        # self.cache = Cache()
    ...
    def popen_recall(self, song_info):
        """
        ...
        def runInThread(song_info):
            ...
            log.info(u"开始播放‘%s’，音量%d。[%d] %s" %
                     (song_info.file_name, self.playing_volume, self.playing_idx + 1, song_info.song_page))
        ...
        #     cache_thread.start()
        thread = threading.Thread(target=runInThread, args=(song_info,))
        thread.start()
    ...
    def play_and_pause(self):
        if not self.playing_flag: return
        if self.pause_flag:
            self.resume()
        else:
            self.pause()
    ...
    def volume_up(self):
        self.playing_volume = self.playing_volume + 7
        if (self.playing_volume > 100):
            self.playing_volume = 100
        if not self.playing_flag:
            return
    ...
    def volume_down(self):
        self.playing_volume = self.playing_volume - 7
        if (self.playing_volume < 0):
            self.playing_volume = 0
        if not self.playing_flag:
            return
        self.set_volume(self.playing_volume)
